...
class ComboBox:
    ...
    def reload(self):
        if self.attributes_button_width == 'auto':
            ...
            if (self.attributes_width // self._buttonEntryParts) < 30:
                self._buttonEntryParts -= 1
            # Calculate the Auto-Button-Width
            self.at_btn_width = self.attributes_width // self._buttonEntryParts
        else:
            # Use the button width provided instead if it ain't 'auto'
        ...
        self.menu.setStyleSheet(f'''
                QMenu {{
                    ...
                    border: {self.attributes_menu_border_width}px solid {self.attributes_menu_border_color};
        ...
        if self.attributes_menu_width  != 'auto': self.menu.setFixedWidth(self.attributes_menu_width); self.menu.atheight = self.attributes_menu_height
        if self.attributes_menu_height != 'auto': self.menu.setFixedHeight(self.attributes_menu_height); self.menu.atwidth = self.attributes_menu_width

        self.ent.setStyleSheet(f'''
            QLineEdit {{
                color: {self.attributes_entry_text_color};
                background-color: {self.attributes_entry_color};
                border-radius: {self.attributes_entry_corner_radius}px;
        ...
        self.btn.setStyleSheet(f"""
            QPushButton {{
                ...
                border-radius: {self.attributes_button_corner_radius};

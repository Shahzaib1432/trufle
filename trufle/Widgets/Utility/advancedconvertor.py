hexes = {'aliceblue': "F0F8FF", 'antiquewhite': "FAEBD7", 'aqua': "00FFFF", 'aquamarine': "7FFFD4",
...
def color_to_rgb(color: str | tuple):
    if isinstance(color, str):
        color = hexes.get(color, color).upper()
        if color.startswith('#'): color = color.removeprefix('#')
        ...
        r, g, b, a = (color[0], color[1], color[2], 255 if len(color) == 3 else color[3])

    else:
        raise TypeError(f'Unknown type of color {color}: {type(color)}')

...
class Entry:
    ...
    def change_text         (self, newText):   self.ent.setText(newText)
    def change_default_text (self, newText):   self.ent.setPlaceholderText(newText)
    def change_text_color   (self, newColor):  self.attributes_text_color = newColor; self.reload_stylesheet()
    def change_entry_color  (self, newColor):  self.attributes_entry_color = newColor; self.reload_stylesheet()    # background-color
    def change_width        (self, newWidth):  self.ent.setFixedWidth(newWidth)
    def change_height       (self, newHeight): self.ent.setFixedHeight(newHeight)
    def change_border_width (self, newWidth):  self.attributes_border_width  = newWidth;  self.reload_stylesheet() # border: {bw}px solid {bc}
    def change_border_color (self, newColor):  self.attributes_border_color  = newColor;  self.reload_stylesheet() # border: {bw}px solid {bc}
    def change_corner_radius(self, newRadius): self.attributes_corner_radius = newRadius; self.reload_stylesheet() # border-radius
    def change_font         (self, newFont):   self.ent.setFont(pv_QFont(newFont, self.attributes_font_size))
    def change_font_size    (self, newSize):   self.ent.setFont(pv_QFont(self.attributes_font, newSize))
    def change_state        (self, newState):  self.ent.setEnabled(True) if newState == 'enabled' else self.ent.setDisabled(True)
